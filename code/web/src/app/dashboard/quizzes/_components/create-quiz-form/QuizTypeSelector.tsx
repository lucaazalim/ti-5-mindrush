...
const quizTypes: QuizOption[] = [
  ...
  { value: "THEME_GENERATED", label: "Gerado por tema", description: "A IA criará perguntas a partir do tema." },
  { value: "PDF_GENERATED", label: "Gerado por PDF", description: "A IA criará perguntas a partir do PDF." }  
];
    ...
    <div className="grid grid-cols-3 gap-4">
      {quizTypes.map((option) => (
        <div
          ...
          className={`cursor-pointer px-4 py-5 border rounded-lg transition-all duration-200 
            ${selectedType === option.value ? "border-blue-500 bg-blue-100 shadow-md" : "border-gray-300 hover:border-gray-400"}
            hover:shadow-lg`}
        >
          <p className="text-lg font-semibold leading-tight pb-1">{option.label}</p>
